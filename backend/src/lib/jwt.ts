...
export function signEmailVerifyToken(userId: string): string {
  return jwt.sign({ userId }, process.env.JWT_EMAIL_VERIFY_SECRET!, {
...
export function verifyEmailToken(token: string): { userId: string } {
  return jwt.verify(token, process.env.JWT_EMAIL_VERIFY_SECRET!) as { userId: string };
}

export function verifyResetToken(token: string): { userId: string } {
  return jwt.verify(token, process.env.JWT_RESET_SECRET!) as { userId: string };
}
