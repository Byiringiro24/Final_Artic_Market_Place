...
async function main() {
  ...
  for (const [topIdx, top] of TREE.entries()) {
    ...
    const topCat = await prisma.category.upsert({
      ...
      update: { icon: top.icon, sortOrder: topIdx },
    });
    ...
    for (const [midIdx, mid] of top.children.entries()) {
      ...
      const midCat = await prisma.category.upsert({
        ...
        update: { parentId: topCat.id, sortOrder: midIdx },
      });
      console.log(`     ↳ ${midName}`);
      ...
      if (typeof mid !== 'string' && mid.children) {
        for (const [leafIdx, leaf] of mid.children.entries()) {
          await prisma.category.upsert({
            ...
            update: { parentId: midCat.id, sortOrder: leafIdx },
          });
          console.log(`        ↳ ${leaf}`);
        }
      }
