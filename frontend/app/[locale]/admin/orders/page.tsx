...
export default function AdminOrdersPage() {
  ...
  const orders     = (data?.data as unknown as ApiData)?.data || (data?.data as unknown as Order[]) || [];
                ...
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {order.items?.[0]?.name}
                      {(order.items?.length ?? 0) > 1 && <span className="text-xs text-gray-400"> +{order.items.length - 1}</span>}
                    </td>
