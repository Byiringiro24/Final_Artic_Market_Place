...
interface Pagination { total: number; totalPages: number; page: number }
...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-orders', page, search, statusFilter, startDate, endDate],
    queryFn: () => {
      ...
      return get<{ data: Order[]; pagination: Pagination }>(`/admin/orders?${params}`);
    },
  });
  ...
  // ApiResponse.paginated wraps: { success, data: Order[], pagination: {...} }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const raw        = data?.data as any;
  const orders: Order[]   = Array.isArray(raw) ? raw : (raw?.data ?? []);
  const pagination: Pagination | undefined = raw?.pagination;
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
              ...
              {isLoading ? (
                ...
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-medium">{fmt(Number(order.totalPrice))}</td>
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t text-sm">
            <span className="text-gray-500">Total: {pagination.total} orders</span>
